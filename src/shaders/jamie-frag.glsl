...
//p is world position of the ray, s is sphere radius
...
vec2 scene(vec3 position) {
	...
	*/
    ...
    sphere_pos.y += sin(position.x + u_Time *0.01) * 0.35; //add time to animate, multiply by samll number to reduce amplitude
    sphere_pos.y += cos(position.z + u_Time * 0.01);
...
void main () {
	...
	out_Col = vec4(diffuseLit, 1.0) * fog; /* applying the fog last */
}
